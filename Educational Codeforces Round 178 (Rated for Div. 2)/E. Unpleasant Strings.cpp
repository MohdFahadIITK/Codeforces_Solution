...
void karke_dekhte_hain(){
    ...
    dp[n]=1;
    //vector<ll> lastfound(k,n);
    vector<vector<ll>> lastfound(k,vector<ll>(n+1,n));
    for(int i=n-1;i>=0;i--){
        ll farthest=i;
        for(int j=0;j<k;j++){
            farthest=max(farthest,lastfound[j][i+1]);
            lastfound[j][i]=lastfound[j][i+1];
        }
        dp[i]=dp[farthest]+1;
        lastfound[s[i]-'a'][i]=i;
    }
    while(q--){
        ...
        ll i=0,j=0;
        ll currpos=-1;
        while(currpos!=n&&j<t2){
            currpos=lastfound[s1[j]-'a'][currpos+1];
            j++;
        }
        cout<<dp[currpos]-1<<endl;
    }
}
