...
void karke_dekhte_hain(){
    ...
    //case <k <k XX
    set<ll> st;
    st.insert(prefsum[n-1]);
    for(int i=n-3;i>=0;i--){
        if(prefsum[i+1]>=0){
            if(st.lower_bound(prefsum[i+1])!=st.end()){
                cout<<"YES\n";
                return;
            }
        }
        st.insert(prefsum[i+1]);
    }
    //case >k <k <k
    b=a;
    reverse(b.begin(),b.end());
    vector<ll> prefsum1(n+1,0);
    for(int i=0;i<n;i++){
        prefsum1[i+1]=prefsum1[i]+b[i];
    }
    set<ll> st1;
    st1.insert(prefsum1[n-1]);
    for(int i=n-3;i>=0;i--){
        if(prefsum1[i+1]>=0){
            if(st1.lower_bound(prefsum1[i+1])!=st1.end()){
                cout<<"YES\n";
                return;
            }
        }
        st1.insert(prefsum1[i+1]);
    }
    //case <k >k <k
    ll pos1=-1;
    for(int i=0;i<n;i++){
