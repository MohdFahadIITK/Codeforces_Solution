...
void karke_dekhte_hain(){
    ...
    if(count<m) cout<<"-1\n";
        ...
        ll ans=prefsum[posi[m]];
        ll counter=0;
        for(int i=0;i<n;i++){
            if(marker[i]==1){
                counter++;
                deb3(counter,posi[m-counter],i);
                if(counter<=m){
                    if(posi[m-counter]>i){
                        deb1(1);
                        deb2(prefsum[posi[m-counter]],prefsum[i+1]);
                        ans=max(ans,prefsum[posi[m-counter]]-prefsum[i+1]);
                    }
                }
            }
        }
