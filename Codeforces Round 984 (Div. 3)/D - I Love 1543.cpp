...
void karke_dekhte_hain(){
    ...
    ll min1=min(n,m);
    ll chikkal=min1/2;
    for(int i=0;i<chikkal;i++){
        ll starti=i,startj=i;
        string curr="";
        for(int t_j=i;t_j<m-i;t_j++){
            curr+=v1[i][t_j];
        }
        for(int t_i=i+1;t_i<n-i;t_i++){
            curr+=v1[t_i][m-i-1];
        }
        for(int t_j=m-i-2;t_j>=i;t_j--){
            curr+=v1[n-i-1][t_j];
        }
        for(int t_i=n-i-2;t_i>i;t_i--){
            curr+=v1[t_i][i];
        }
        curr+=curr[0];
        curr+=curr[1];
        curr+=curr[2];
        for(int i=3;i<curr.size();i++){
            if(curr[i-3]=='1'&&curr[i-2]=='5'&&curr[i-1]=='4'&&curr[i]=='3'){
                count++;
            }
        }
    }
