...
void karke_dekhte_hain(){
    ...
    if(getposfromstart[m-1]!=-1){
        cout<<"0\n";
    }
    else{
        vector<ll> getposfromend(m,-1);
        i=n-1;
        j=m-1;
        while(i>=0&&j>=0){
            if(a[i]>=b[j]){
        ...
        ll anspossible=INT_MAX;
        ll cnt=0;
        for(int i=1;i<m-1;i++){
            if(getposfromstart[i-1]!=-1){
                if(getposfromstart[i-1]<getposfromend[i+1]){
                    anspossible=min(anspossible,b[i]);
                }
            }
        }
        if(getposfromend[1]!=-1) anspossible=min(anspossible,b[0]);
        if(getposfromstart[m-2]!=-1) anspossible=min(anspossible,b[m-1]);
        if(anspossible==INT_MAX) anspossible=-1;
        cout<<anspossible<<endl;
    }
    //cout<<endl;
}
