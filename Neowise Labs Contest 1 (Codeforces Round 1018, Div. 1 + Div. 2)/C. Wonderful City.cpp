...
void karke_dekhte_hain(){
    ...
    ll INT_MAX1=1e15;
    vector<ll> dp_take(n,INT_MAX1);
    vector<ll> dp_nottake(n,INT_MAX1);
    dp_take[0]=a[0];
    dp_nottake[0]=0;
    ll ans=0;
    for(int i=1;i<n;i++){
        ...
        int flag=0;
        for(int j=0;j<n;j++){
            if(v[i][j]==v[i-1][j]) flag=1;
        }
        //lasttake-thisnottake
        int flag2=0;
        for(int j=0;j<n;j++){
            if(v[i][j]==v[i-1][j]+1) flag2=1;
        }
        if(flag==0){
            if(flag2==0){
                dp_take[i]=min(dp_take[i],dp_take[i-1]+a[i]);
                dp_nottake[i]=min(dp_nottake[i],dp_take[i-1]);
            }
            else{
                dp_take[i]=min(dp_take[i],dp_take[i-1]+a[i]);
            }
        ...
        //lastnottake-thistake
        int flag3=0;
        for(int j=0;j<n;j++){
            if(v[i][j]==v[i-1][j]-1) flag3=1;
        }
        //lastnottake-thisnottake
        if(flag3==0){
            if(flag==0){
                dp_take[i]=min(dp_take[i],dp_nottake[i-1]+a[i]);
                dp_nottake[i]=min(dp_nottake[i],dp_nottake[i-1]);
            }
            else{
                dp_take[i]=min(dp_take[i],dp_nottake[i-1]+a[i]);
            }
    ...
    if(dp_take[n-1]==INT_MAX1&&dp_nottake[n-1]==INT_MAX1){
        cout<<"-1\n";
        return;
    }
    ans+=min(dp_take[n-1],dp_nottake[n-1]);
    for(int i=0;i<n;i++){
        dp_take[i]=INT_MAX1;
        dp_nottake[i]=INT_MAX1;
    }
    dp_take[0]=b[0];
    dp_nottake[0]=0;
    for(int j=1;j<n;j++){
        ...
        int flag=0;
        for(int i=0;i<n;i++){
            if(v[i][j]==v[i][j-1]) flag=1;
        }
        //lasttake-thisnottake
        int flag2=0;
        for(int i=0;i<n;i++){
            if(v[i][j]==v[i][j-1]+1) flag2=1;
        }
        if(flag==0){
            if(flag2==0){
                dp_take[j]=min(dp_take[j],dp_take[j-1]+b[j]);
                dp_nottake[j]=min(dp_nottake[j],dp_take[j-1]);
            }
            else{
                dp_take[j]=min(dp_take[j],dp_take[j-1]+b[j]);
            }
        ...
        int flag3=0;
        for(int i=0;i<n;i++){
            if(v[i][j]==v[i][j-1]-1) flag3=1;
        }
        //lastnottake-thisnottake
        if(flag3==0){
            if(flag==0){
                dp_take[j]=min(dp_take[j],dp_nottake[j-1]+b[j]);
                dp_nottake[j]=min(dp_nottake[j],dp_nottake[j-1]);
            }
            else{
                dp_take[j]=min(dp_take[j],dp_nottake[j-1]+b[j]);
            }
